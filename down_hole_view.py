...
class dhView(QWidget):
    ...
    def __init__(self, parent):
        ...
        addButton = QPushButton(buttonBar)            # adds images to zoomIm class in content widget
        addButton.setText('Add')
        addButton.clicked.connect(lambda: self.addCores())
        clearButton = QPushButton(buttonBar)           # removes images to zoomIm class in content widget
        ...
        self.meterScroll.verticalScrollBar().setMaximum(self.zoomIm.verticalScrollBar().maximum())

        # link the meter scrollbar with the zoomIm scrollbar
        self.zoomIm.verticalScrollBar().valueChanged.connect(self.meterScroll.verticalScrollBar().setValue)
        self.meterScroll.verticalScrollBar().valueChanged.connect(self.zoomIm.verticalScrollBar().setValue)
        ...
        self.zoomIm.horizontalScrollBar().valueChanged.connect(self.headerAreaScroll.horizontalScrollBar().setValue)
    ...
    def setMeter(self, vals, numIms):
        ...
        for i in range(0,len(vals),int(len(vals)/numIms)):
            cbVal = "{:.1f}".format(vals[i]) + ' m'  # formats depth text to one decimal place and adds 'm'
            if  i == 0:
    ...
    def addCores(self):
        ...
        """
        self.typeList.append(self.typeCB.currentText())      # add the image's type to the typeList
    ...
    def addImages(self):
        ...
        self.addZoomMeter()
        self.headerArea.setFixedWidth(self.zoomIm.mapFromScene(self.zoomIm.sceneRect()).boundingRect().width())
        self.addHeader(int(self.zoomIm.mapFromScene(self.zoomIm.sceneRect()).boundingRect().width()/len(self.typeList)))
    ...
    def drawMeter(self, width, height, fontSize):
        ...
        for i in range(numPixmaps):
            ...
            while imNum*ySpace < pixmap[i].height() + i*32767 and imNum < numIms:
                ...
                qp.drawText(QPoint(0, imNum*ySpace-i*32767+int(1.5*fontSize)), meterVals[imNum])
                imNum = imNum + 1
    ...
    def addZoomMeter(self):
        ...
        meterHeight = self.zoomIm.mapFromScene(self.zoomIm.sceneRect()).boundingRect().height()
    ...
    def addHeader(self, width):
        ...
        # remove headers
        if self.headerAreaLayout.count() > 2:          # > 2 because header area has two spacers at each end
        ...
        self.headerAreaScroll.setFixedWidth(self.zoomIm.width()-self.zoomIm.verticalScrollBar().width())
    ...
    def makeHeader(self,width, min, type, idx):
        ...
        windowHeaderLayout.addWidget(closeButton)  # connect to closeButton which removes it from zoomIm
    ...
    def imageZoomed(self):
        ...
        self.addZoomMeter()
        self.addHeader(int(self.zoomIm.mapFromScene(self.zoomIm.sceneRect()).boundingRect().width()/len(self.typeList))-2)
        self.headerArea.setFixedWidth(self.zoomIm.mapFromScene(self.zoomIm.sceneRect()).boundingRect().width())
        self.zoomIm.horizontalScrollBar().setMaximum(10000)  # set maximum value of each scroll bar so that they scroll evenly together
        self.headerAreaScroll.horizontalScrollBar().setMaximum(10000)
        self.headerAreaScroll.horizontalScrollBar().setValue(self.zoomIm.horizontalScrollBar().value())
...
class zoomImage(QGraphicsView):
    ...
    def __init__(self, parent):
        ...
        self.photo = QGraphicsPixmapItem()                # initialize the QGraphicsPixmapItem, this will make up the image
    ...
    def fitInView(self):
        ...
        if not rect.isNull():
            ...
            if not self.empty:
                ...
                if scenerect.height() < scenerect.width() and scenerect.height() < 0.75*viewrect.height():
                ...
                if self.mapFromScene(self.sceneRect()).boundingRect().height() < 25:
                    factor = 25/self.mapFromScene(self.sceneRect()).boundingRect().height()
                    self.scale(factor, factor)
    ...
    def wheelEvent(self, event):
        ...
        if not self.empty:
            if event.delta() > 0:
                ...
                if  height < 1e+6 and height < 10*self.imHeight:
                    factor = 1.25
                    self.zoom += 1
                else:
                    factor = 1
            ...
            self.viewport().setGeometry(0, 0, self.imWidth*factor, self.imHeight*factor)
            self.zoomed.emit()
