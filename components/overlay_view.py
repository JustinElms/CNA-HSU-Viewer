...
class overlayView(QWidget):
    ...
    def setMinColors(self, inColors):
        ...
        for i in range(len(inColors)):
            ...
            rgb = tuple(
                int(hex[j : j + 2], 16) for j in (0, 2, 4)
            )  # convert hex to rgb
            rgb = (np.asarray(rgb) * (255 / np.max(rgb))).astype(
                np.uint8
            )  # convert tuple to numpy int array
    ...
    def overlayIms(self):
        ...
        # get core box image
        cbIm = (
            self.cbIms[self.boxCB.currentIndex()].scaledToWidth(self.width() - 350)
        ).toImage()
        ...
        for i in range(len(self.compMinList)):
            if self.visButtons[i].isChecked():
                ...
                if (
                    self.opSliders[i].value() > maxSliderVal
                ):  # change maxSliderVal if current slider larger
                    maxSliderVal = self.opSliders[i].value()

                minIm = self.compIms[self.compMinList[i]]["Images"][
                    idx
                ]  # get comp spec image
                minIm = minIm.scaledToWidth(
                    cbDim[1]
                ).toImage()  # scale to core box image width
                ...
                minImOl = (self.opSliders[i].value() / 100) * np.multiply(
                    minIm, self.colors[i][:]
                )

                # add minIm to the overlay
                overlay[startRow : startRow + minDim[0], :, :] = (
                    overlay[startRow : startRow + minDim[0], :, :] + minImOl
                )
        ...
        cbIm[nzIdx] = (
            flatIm[nzIdx[0], nzIdx[1]] * ((100 - (maxSliderVal)) / 100) + overlay[nzIdx]
        )  # replace pixels of corebox image underneath overlay with grayscale image and overlay
