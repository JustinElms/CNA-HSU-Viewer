...
def drawMeter(depthValues, width, height, meterLoaded):
    ...
    for i in range(len(depthValues + 1)):
        ...
        if i == 0 and pxNum == 0:  # starting major tick and text
            ...
            qp.drawRect(
                int(0.25 * width),
                i * tickSpace + 20 - pxNum * 32767,
                int(0.75 * width),
                1,
            )
            if meterLoaded:
                qp.drawText(
                    QPoint(0, i * tickSpace - pxNum * 32767 + 17),
                    "{:.1f}".format(depthValues[i]) + " m",
                )
            else:
                qp.drawText(
                    QPoint(0, i * tickSpace - pxNum * 32767 + 17),
                    "{:.0f}".format(depthValues[i]),
                )
...
def drawSpecPlot(width, height, plotSpec, axisLims, plotDepth, plotColor, meterVals):
    ...
    plotFig = Figure(
        figsize=(width / 100, (height - 40) / 100), dpi=100, facecolor="#000000"
    )
...
def drawStackPlot(width, height, plotSpec, specDim, plotColor, minMeter, meterVals):
    ...
    for i in range(specDim[1] + 1):
        if i == 0:
            ...
            plot.fill_betweenx(
                minMeter, plotSpec[:, i], plotSpec[:, i - 1], facecolor=plotColor[i]
            )
        else:
            plot.fill_betweenx(
                minMeter, plotSpec[:, i], plotSpec[:, i - 1], facecolor="#969696"
            )
